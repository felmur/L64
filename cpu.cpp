...
extern uint8_t kernal[8192];
extern uint8_t basic[8192];
...
CPU::CPU(QObject *parent) : QThread(parent)
    ...
    , nmi_line(true)
{
    Write = (BusWrite) &MemoryWrite;
    Read = &MemoryRead;
    Cycle = (ClockCycle) &tick;
...
#define MAKE_INSTR(HEX, CODE, MODE, CYCLES, PENALTY) \
    instr.code = &CPU::Op_ ## CODE; \
        instr.scode = # CODE; \
        instr.addr = &CPU::Addr_ ## MODE; \
        instr.saddr = # MODE; \
        instr.cycles = CYCLES; \
...
CPU::~CPU()
{
    cout << "[CPU] Deleting CPU thread..." << endl;
}
...
void CPU::RunEternally()
{
    ...
    while(!illegalOpcode)
    {
        ...
        // run clock cycle callback
        if (Cycle)
            for(int i = 0; i < instr.cycles; i++) Cycle(this);
    }
}
...
void CPU::run()
{
    ...
    while(!illegalOpcode && !isInterruptionRequested())
    {
        CheckInterrupts();
    ...
    if (illegalOpcode){
        printf("[CPU] Illegal OpCode $%02x found at address $%04x\n",opcode,pc-1);
    }
    cout << "[CPU] CPU thread stopped" << endl;
}
...
uint8_t CPU::MemoryRead(uint16_t address)
{
    ...
    b0 = memory[0x0001] & 0x01;
    b1 = memory[0x0001] & 0x02;
    b2 = memory[0x0001] & 0x04;

    val = memory[address];

    if(b2 && (b1 || b0)){
        if ((address >=0xD000 && address<=0xDFFF)) val = ioarea[address-0xD000];
        if ((address >=0xD000 && address<=0xD3FF)) val = vic->Peek(address);
        if (address>=0xDC00 && address<=0xDCFF) val = cia1->Peek(address);
        if (address>=0xDD00 && address<=0xDDFF) val = cia2->Peek(address);
    }
    if(!b2 && (b1 || b0)) {
        if ((address >=0xD000 && address<=0xDFFF)) val = chars[address-0xD000];
    }

    if (!b1 && b0){
        if ((address >=0xA000 && address <=0xBFFF)) val = memory[address];
        else if ((address >=0xE000)) val = memory[address];
    }
    else if (b1 && !b0){
        if ((address >=0xA000 && address <=0xBFFF)) val = memory[address];
        else if ((address >=0xE000)) val = kernal[address-0xE000];
    }
    else if (b1 && b0){
        if ((address >=0xA000 && address <=0xBFFF)) val = basic[address-0xA000];
        else if ((address >=0xE000)) val = kernal[address-0xE000];
    }
...
void CPU::MemoryWrite(uint16_t address, uint8_t value)
{
    ...
    if((b2 && (b1 || b0)) || memory[0x0001] == 0){
        if (address>=0xDC00 && address<=0xDCFF) cia1->Poke(address,value);
        else if (address>=0xDD00 && address<=0xDDFF) cia2->Poke(address,value);
        else if (address>=0xD000 && address<=0xD3FF) vic->Poke(address,value);
        else if (address>=0xD400 && address<=0xD41F) sid->Poke(address,value);
        if ((address >=0xD000 && address<=0xDFFF)) {
            ioarea[address-0xD000]=value;
            return;
        }
    }
...
{
    (void) m;
    while(thisCPU->stoptick) {};
    thisCPU->stoptick = true;
}

uint8_t CPU::unlocktick()
{
    stoptick=false;
    return cicli;
}
...
void CPU::setIllegalOpcode(bool newIllegalOpcode)
{
    illegalOpcode = newIllegalOpcode;
}
...
void CPU::SetS (uint8_t n)
{
    sp = n;
}

void CPU::SetP (uint8_t n)
{
    status = n;
}

void CPU::SetA (uint8_t n)
{
    A = n;
}

void CPU::SetX (uint8_t n)
{
    X = n;
}

void CPU::SetY (uint8_t n)
{
    Y = n;
}
...
void CPU::Op_SAX(uint16_t src)
{
    Write(src, A & X); // most simple illegal here
}

void CPU::Op_SBX(uint16_t src)
{
    uint16_t m = Read(src);
    uint8_t tmp = A & X;
